...
$sql = "SELECT * FROM phone_status_table LEFT JOIN uuid ON phone_status_table.androidId = uuid.androidId WHERE uuid.phname LIKE 'T%' ORDER BY phname ASC;";
        ...
        if ($previous_phname == null || $number - $previous_phname == 1) {
            ...
            for ($i = 1; $i < $num; $i++) {
                $results[] = array(
                    ...
                    // 빈 번호를 추가
                    'phname' => 'ERROR-' . str_pad($previous_phname + $i, 2, '0', STR_PAD_LEFT),
                );
            }
