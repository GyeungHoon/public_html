...
if (isset($_POST['uname']) && isset($_POST['password'])) {
	...
	if (empty($uname)) {
		...
		if (mysqli_num_rows($result) === 1) {
			...
			if ($row['id'] === $uname && $row['psword'] === $pass) {
				...
				if ($_SESSION['id'] == "authority_ic_031") {
					...
					$_SESSION['login_log'] = $url_parm;
					// $url = "../admin_hi/select_query.php?key=$url_parm";
					$url = "../admin_hi/select_query.php";
					...
					$_SESSION['id'] = $uname;
				echo ("<script>location.replace('../receive/receive.php');</script>");
			} else {
	...
	header("Location: receive/receive.php");
	exit();
}
